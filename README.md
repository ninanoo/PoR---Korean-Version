...
###### ※ 이 알고리즘은 코인(coin) 발행을 목적으로 고안되지는 않았습니다.<br/>알고리즘을 구현하고 검증하는데 필요한 자금을 확보하기 위해 코인 발행이 필요할 수도 있습니다.<br/>코인이 발행되더라도 사익이 아니라 알고리즘의 개념증명(proof of concept)이 목적이 되어야 합니다.<br/>보상 체계는 필요하고 이는 여타 스마트계약(smart contract) 관련 알고리즘들의 방법을 따릅니다.<br/>이 알고리즘이 블록체인과 관련된 많은 난제들을 해결하여 블록체인 기술이 활성화되길 바랍니다.
