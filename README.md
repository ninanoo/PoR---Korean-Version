...
하위 블록들에 관련성은 체인에 추가된 순서에 따라 지수 분포로 감소하는 배율이 사용됩니다.
...
이는 반대로 상대적으로 낮은 자체 관련성을 갖는 블록이 먼저 추가가 되고 그 뒤로 다수의 하위 블록이 연이어 추가가 되어 있으면 더 좋은 자체 관련성을 갖고 있는 블록일지라도 합의에서 질 수 있습니다.
...
알고리즘에서 정하는 부분은 단순히 소유정보로 부터 만들어진 해시값을 이전 해시값과 비트순으로 비교하는 것이 전부입니다.
