...
이중체인의 사용으로 인해 인증을 생성할 수 있도록 합의를 통해 등록된 후보는 매우 긴 시간 이후에 실제 인증을 생성하고 이때도 경쟁이 이루어집니다.
...
그러나 관련성에 의한 증명에서는 새로 추가된 블록이 인증을 발행하기까지는 매우 긴 시간동안 많은 합의를 거치므로 모든 네트워크 노드의 검증을 거치게 됩니다.
...
소유정보는 각 노드를 유일하게 식별(identification)할 수 있어야 하고 모든 노드에 의해 인증(authentication)될 수 있어야 합니다.
소유정보를 무엇으로 어떻게 정의하냐에 따라 식별에 대상이 단순히 노드가 아니라 실세계의 개인이 될 수도 있고 각 나라나 조직의 구성체계 등에 맞추어 구조화 시킬 수도 있습니다.
...
그런데 여기서는 공개키와 개인키 쌍의 소유키를 관련성을 만드는 소유정보로도 사용하였습니다.
알고리즘에서 정하는 부분은 단순히 소유정보를 포함하여 만들어진 해시값을 이전 해시값과 비트순으로 비교하는 것이 전부입니다.
이 문서의 목적은 소유정보에 대한 체계화가 아니라 알고리즘의 기술적인 기본 동작과 그에 대한 검증입니다.
